...
#	test[0].current = true
	...
	if is_instance_valid(current_active):
		...
		ui.move_button.disabled = not current_active.is_network_master()
		ui.attack_button.disabled = not current_active.is_network_master()
		ui.set_end_turn_button(current_active.is_network_master() and end_turn_queue == 0 and not current_active.has_node("AiControl"))
...
#					new_actor.team = Actor.TEAMS.ENEMIES
	...
	if not new_actor.is_network_master():
		new_actor.set_ring_color(Color.red)
	...
	default_spell_scene.singleplayer = false
...
#		ui.set_end_turn_button(true)
	...
	#TODO : clean up the old ui (spells bar)
...
#	yield(get_tree().create_timer(1.5),"timeout")
	ui.set_end_turn_button(current_active.is_network_master())
...
#		index = new_puppet.get_index()
	...
	for spell_scene in new_puppet.spells_node.get_children():
		...
		spell_scene.connect("push_requested", self, "on_push_requested")
		spell_scene.connect("spell_cast", self, "on_spell_cast")
		spell_scene.singleplayer = false
...
func on_actor_pressed(actor):
	if not selected_spell: 
		var preview = actor != current_active or not actor.is_network_master()
...
func on_back_button_pressed() -> void:
	ui.set_preview_actor(current_active, not current_active.is_network_master())
...
func on_slide_requested(target_position : Vector2, actor : Actor, strength : int = 1,  duration : float = 0.5) -> void:
	rpc("slide", target_position, actor.get_path(), strength, duration)
...
func on_push_requested(origin : Vector2, caster : Actor, target : Actor, strength : int = 1, duration : float = 0.5) -> void:
	rpc("push", origin, caster.get_path(), target.get_path(), strength, duration)
...
func on_spell_requested(spell : Spell, spell_unique_name : String) -> void:
	...
	if not actor.is_network_master():
		return
...
func on_spell_cast(_spell : Spell, actor : Actor = current_active, target_dict : Dictionary = {}, crit : bool = false) -> void:
	...
	var animation_name : String = "Explosion"
...
#		animation_name = _spell.animation
	...
	if animation_name == "Explosion":
		...
		spells_fx.rset("cast_positions", pos_array.duplicate())
		spells_fx.rset("draw_cast_cells", true)
		
		for pos in pos_array:
			spells_fx.rpc("play_anim", animation_name, pos, _spell.spell_type)
	else:
		
		spells_fx.rpc("play_anim", animation_name, animation_pos)
...
func on_actor_died(actor):
	...
	for actor_ in actors.get_children():
		if actor_ is Puppet or actor_ == actor :
			continue
	...
	if not actor is Puppet:
		...
		if td_index <= turn_displayer_index:
			turn_displayer_index -= 1
	
	if actor_index < active_index or (actor is Puppet and actor_index == active_index):
		active_index -= 1
...
func on_title_screen_pressed() -> void:
